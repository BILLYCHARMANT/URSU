...
export function EditCourseModal({
  ...
  // Load course data to determine initial step and programId
  useEffect(() => {
    fetch(`/api/courses/${courseId}`)
      ...
      .then((course) => {
        ...
        // Always start from step 1 when editing so user can review course details first
        // They can navigate to other steps using the wizard navigation
        setStep(1);
        setLoading(false);
      })
      .catch(() => {
  ...
  if (loading) {
    return (
      ...
      >
        <div
          className="rounded-2xl border border-[#e5e7eb] dark:border-[#374151] bg-white dark:bg-[#1f2937] shadow-xl p-6"
          onClick={(e) => e.stopPropagation()}
        >
          <p className="text-sm text-[#6b7280] dark:text-[#9ca3af]">Loading course data...</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e7eb] dark:border-[#374151]">
          <h2 className="text-xl font-bold text-[#171717] dark:text-[#f9fafb]">Edit Course</h2>
          <button
            onClick={handleClose}
