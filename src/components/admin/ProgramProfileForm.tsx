...
export function ProgramProfileForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Redirect to program detail page where user can create courses
      router.push(`/dashboard/admin/programs-management/${data.id}`);
      router.refresh();
    } catch {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-[#e5e7eb] dark:border-[#374151] bg-white dark:bg-[#1f2937] p-6 shadow-sm space-y-4">
        <div>
